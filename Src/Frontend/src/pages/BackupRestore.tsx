...
export function BackupRestore() {
  ...
  const handleRestore = (backup: BackupEntry) => {
    ...
    if (
      confirm(`Phục hồi dữ liệu từ bản sao lưu ${backup.id} (${backup.date})?`)
    ) {
  ...
  const backupColumns = [
    ...
    },
    {
      ...
      render: (val: string) => (
        <span
          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${val === 'Tự động' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}
        >
          {val}
        </span>
      ),
    },
    {
      ...
      render: (val: string) => (
        <span
          className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${val === 'Thành công' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
        >
  ...
  const backupActions = [
    {
      key: 'restore',
      label: 'Phục hồi',
      onClick: (row: BackupEntry) => handleRestore(row),
    },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:bg-green-400 rounded-lg transition-colors"
          >
            {isBackingUp ? (
              <>
      ...
      <DataTable
        ...
        data={backups}
        searchPlaceholder="Tìm mã backup, trạng thái, loại..."
        rowActions={backupActions}
      />
    </div>
  )
}
