// @ts-nocheck
import { AlertTriangleIcon, CircleAlertIcon, ClockIcon, FileTextIcon, LayoutGridIcon, ListIcon, PackageSearchIcon, RefreshCcwIcon, TrendingUpIcon } from 'lucide-react';
...
import { getProductImageSrc as _getProductImageSrc } from '../utils/productImage';
const numberFormatter = new Intl.NumberFormat('vi-VN');
function formatMoney(value) {
  return `${numberFormatter.format(value || 0)} đ`;
}

export function WatchCategories({ lowStockOnly = false, targetLowStockProductId = null, onConsumeTargetLowStock, onClearLowStockFilter, readOnly = false }) {
  const { can } = usePermission();
  // readOnly = true khi user chỉ có quyền xem (vd: sales)
  const isSales = readOnly;
  ...
  const getProductImageSrc = (product) => {
    if (!product) return '';
    return _getProductImageSrc(product.image || '', product.name || '', product.brand || '');
  };
  const columns = [
    { key: 'id', label: 'Mã SP' },
    {
  ...
  const rowActions = isSales
    ...
    : [
      ...
      ...(can('sanpham', 'delete') ? [{
        ...
        label: 'Xóa',
        onClick: (row) => {
          if (row.status === 'Tạm ẩn') return;
          handleDelete(row);
        },
        disabled: (row) => row.status === 'Tạm ẩn',
  ...
  const handleDelete = (row) => {
    if (Number(row.stock || 0) > 0) {
      showNotice(`Không thể xóa "${row.name}" vì còn ${row.stock} sản phẩm trong kho.`, 'error');
      return;
    }
  ...
  return (<div className="space-y-4">
    ...
    {!isSales && can('thongke', 'view') && <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
    ...
    <div className="rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {isLowStockMode && can('phieunhap', 'create') ? (<button
            type="button"
    ...
    <DataTable
      ...
      {...(!isSales && can('sanpham', 'create') ? { onAdd: openAdd, addLabel: 'Thêm sản phẩm' } : {})}
    ...
    <div className="space-y-3">
      ...
      {locationViewMode === 'table' ? (
        <DataTable
          ...
          data={locationRows}
          searchPlaceholder="Tìm kiếm..."
          {...(!isSales && can('sanpham', 'create') ? { onAdd: openAddLocation, rowActions: locationRowActions, addLabel: 'Thêm vị trí' } : {})}
          pageSize={5}
        />
      ) : (
        <div className="space-y-3">
          {!isSales && can('sanpham', 'create') && (
    ...
    <Modal isOpen={detailModalOpen} onClose={() => setDetailModalOpen(false)} title={`Chi tiết ${viewingProduct?.name || ''}`} size="xl">
      <div className="grid gap-4 md:grid-cols-[280px_1fr]">
        <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
          {getProductImageSrc(viewingProduct) ? (
            <img
              src={getProductImageSrc(viewingProduct)}
              alt={viewingProduct?.name || 'Ảnh sản phẩm'}
              className="h-64 w-full rounded-xl border border-gray-200 object-cover bg-white"
            />
          ) : (
            <div className="h-64 w-full rounded-xl border border-dashed border-gray-200 bg-gray-100 flex flex-col items-center justify-center gap-2">
              <span className="text-base font-bold text-gray-500 text-center px-4 leading-tight">
                {viewingProduct?.brand || 'Chưa có ảnh'}
              </span>
            </div>
          )}
        </div>
