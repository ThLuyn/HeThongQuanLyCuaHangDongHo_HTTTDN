...
export function WatchCategories() {
  ...
  const [reportMonth, setReportMonth] = useState<number | null>(now.getMonth() + 1)
  ...
  const [form, setForm] = useState({
    ...
    status: 'Hoạt động' as ProductRow['status'],
  })
  ...
  const columns = [
    ...
    { key: 'displayPosition', label: 'Vị trí trưng bày' },
    {
      key: 'importPrice',
      label: 'Giá nhập',
      render: (val: number) => <span className="text-gray-700">{formatMoney(val)}</span>,
    },
    {
      key: 'sellPrice',
      label: 'Giá bán',
    ...
    },
    {
      key: 'status',
      label: 'Trạng thái',
      render: (val: ProductRow['status']) => (
        <span
          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
            val === 'Hoạt động' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700'
          }`}
        >
          {val}
        </span>
  ...
  const rowActions = [
    ...
    },
    {
      key: 'edit',
      label: 'Sửa',
      onClick: (row: ProductRow) => openEdit(row),
      className: 'p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors',
    },
    {
      key: 'delete',
      label: 'Xóa',
      onClick: (row: ProductRow) => handleDelete(row),
  ...
  const loadProductsAndSuppliers = async () => {
    try {
      ...
      const mappedProducts: ProductRow[] = productRows.map((item: ProductItem) => ({
        msp: Number(item.MSP),
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              type="number"
              value={reportYear}
              onChange={(e) => setReportYear(Number(e.target.value) || now.getFullYear())}
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 text-sm text-white bg-gold-500 hover:bg-gold-600 rounded-lg"
            >
      ...
      <DataTable
        ...
        advancedFilterKeys={['brand', 'supplier']}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                {suppliers.map((s) => (
                  <option key={s.MNCC} value={s.MNCC}>
                    {s.TEN}
                  </option>
                ))}
              </select>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, displayPosition: e.target.value }))}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, productionYear: e.target.value ? Number(e.target.value) : null }))}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg"
              />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, sellPrice: Number(e.target.value) || 0 }))}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <select
              value={form.status}
              onChange={(e) => setForm((prev) => ({ ...prev, status: e.target.value as ProductRow['status'] }))}
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg"
            >
      ...
      <Modal
        ...
        title={`Chi tiết ${viewingProduct?.name || ''}`}
      >
