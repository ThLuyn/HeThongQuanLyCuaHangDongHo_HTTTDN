...
const columns = [
  ...
  },
  {
    ...
    render: (val: string) => (
      <span
        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${val === 'Hoàn thành' ? 'bg-green-100 text-green-700' : val === 'Đang giao' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}
      >
        {val}
      </span>
    ),
  ...
  return (
    <>
      <DataTable
        ...
        columns={columns}
        data={receipts}
        searchPlaceholder="Tìm phiếu xuất..."
        onAdd={openAdd}
        onDelete={handleDelete}
        addLabel="Tạo phiếu xuất"
      />
      <Modal
        isOpen={modalOpen}
