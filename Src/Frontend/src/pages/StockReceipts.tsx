...
const columns = [
  ...
  { key: 'qtyTotal', label: 'Tổng SL' },
    ...
    },
    {
        key: 'status',
        label: 'Trạng thái',
    render: (val) => (<span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${val === 'Đã duyệt'
...
export function StockReceipts() {
  ...
  const [successMessage, setSuccessMessage] = useState('');
    ...
    useEffect(() => {
      if (!mncc) {
        setItems((prev) => prev.map((line) => ({ ...line, msp: '' })));
        return;
      }
    ...
    return (<>
      ...
      {loading ? (<div className="rounded-xl border border-gray-100 bg-white p-6 text-center text-gray-600 shadow-sm">
        ...
        </div>) : (<DataTable title="Phiếu nhập kho" columns={columns} data={viewRows} searchPlaceholder="Tìm phiếu nhập..." advancedFilterKeys={["supplier", "employee", "status", "date", "total", "productCount", "qtyTotal"]} forceSelectFilterKeys={["supplier", "employee", "status"]} rangeFilterKeys={[
          ...
          <div className="space-y-2">
            ...
            return (<div key={index} className="grid grid-cols-12 gap-3 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
                <div className="col-span-12 md:col-span-5">
                  ...
                  <select value={line.msp} onChange={(e) => updateLine(index, { msp: e.target.value ? Number(e.target.value) : '' })} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/50" disabled={!mncc}>
                    <option value="">{mncc ? "Chọn sản phẩm" : "Chọn nhà cung cấp trước"}</option>
                    {filteredProducts.map((p) => (<option key={p.MSP} value={p.MSP}>
                        {p.TEN} (MSP: {p.MSP})
                      </option>))}
                  </select>
                </div>
