    ...
    render: (val: string) => (
      <span
        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
          val === 'Đã duyệt' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
        }`}
      >
        {val}
      </span>
...
export function StockReceipts() {
  ...
  const [receipts, setReceipts] = useState<ImportReceiptItem[]>([])
  const [suppliers, setSuppliers] = useState<SupplierItem[]>([])
  const [products, setProducts] = useState<ProductItem[]>([])

  const [modalOpen, setModalOpen] = useState(false)
  const [saving, setSaving] = useState(false)
  const [mncc, setMncc] = useState<number | ''>('')
  const [items, setItems] = useState<ReceiptItemForm[]>([{ msp: '', sl: 1, tienNhap: 0 }])
  ...
  const viewRows: ReceiptViewRow[] = useMemo(() => {
  ...
  const updateLine = (index: number, patch: Partial<ReceiptItemForm>) => {
      ...
      <Modal isOpen={modalOpen} onClose={closeCreate} title="Tạo phiếu nhập kho" size="lg">
        <div className="space-y-4">
          <div>
            ...
            <select
              value={mncc}
              onChange={(e) => setMncc(e.target.value ? Number(e.target.value) : '')}
          ...
          <div className="space-y-2">
            ...
            {items.map((line, index) => (
              <div key={index} className="grid grid-cols-12 gap-2 rounded-lg border border-gray-100 bg-gray-50 p-3">
                <div className="col-span-12 md:col-span-5">
                  <label className="mb-1 block text-xs font-medium text-gray-600">Sản phẩm</label>
                  <select
                    value={line.msp}
                    onChange={(e) => updateLine(index, { msp: e.target.value ? Number(e.target.value) : '' })}
                ...
                <div className="col-span-6 md:col-span-2">
                  ...
                  <input
                    ...
                    min={1}
                    value={line.sl}
                    onChange={(e) => updateLine(index, { sl: Number(e.target.value) || 0 })}
                    className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/50"
                  />
                </div>
                ...
                <div className="col-span-6 md:col-span-3">
                  ...
                  <input
                    ...
                    min={1}
                    value={line.tienNhap}
                    onChange={(e) => updateLine(index, { tienNhap: Number(e.target.value) || 0 })}
                    className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-400/50"
                  />
                </div>
