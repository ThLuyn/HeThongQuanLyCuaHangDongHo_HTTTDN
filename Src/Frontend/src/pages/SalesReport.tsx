// @ts-nocheck
import { CalendarDaysIcon, DollarSignIcon, FileTextIcon, PercentIcon, ShoppingCartIcon, TrendingUpIcon, } from 'lucide-react';
...
function splitEntryByProducts(entry) {
...
function createReport(fromDate, toDate, reportName) {
    const filtered = salesEntries.filter((entry) => entry.date >= fromDate && entry.date <= toDate);
    ...
    const secondHalfRevenue = filtered
        .filter((item) => item.date > pivotDate)
        .reduce((sum, item) => sum + item.revenue, 0);
    const growth = firstHalfRevenue === 0 ? 0 : ((secondHalfRevenue - firstHalfRevenue) / firstHalfRevenue) * 100;
    const categoryRevenueMap = filtered.reduce((acc, item) => {
    ...
    const monthlyRevenueMap = filtered.reduce((acc, item) => {
        const monthKey = toMonthKey(item.date);
        acc[monthKey] = (acc[monthKey] || 0) + item.revenue;
        return acc;
    }, {});
    const monthlyOrdersMap = filtered.reduce((acc, item) => {
        const monthKey = toMonthKey(item.date);
...
export function SalesReport() {
    ...
    const initialReport = createReport('2026-01-01', '2026-12-31', 'Báo cáo tổng quan năm 2026');
    ...
    const activeReport = reports.find((item) => item.id === selectedReportId) || reports[0] || null;
    ...
    if (!activeReport) {
        return (<div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 text-gray-600">
    ...
    const monthlyColumns = [
        ...
        },
        {
            ...
            render: (val) => {
                ...
                return (<span className={`font-medium ${growthValue > 0 ? 'text-green-600' : growthValue < 0 ? 'text-red-600' : 'text-gray-500'}`}>
            {val}
          </span>);
            },
        },
    ...
    const profitColumns = [
        ...
        },
        {
            key: 'profitText',
            label: 'Lợi nhuận',
            render: (val, row) => (<span className={row.profit >= 0 ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>{val}</span>),
        },
        {
            key: 'marginText',
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-xl px-6 py-4 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            ...
            <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400/50"/>
          </div>

          <div>
            <label className="block text-sm text-gray-600 mb-1.5">Đến ngày</label>
            <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400/50"/>
          </div>

          <div className="flex items-end">
            <button onClick={handleCreateReport} className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gold-500 hover:bg-gold-600 rounded-lg transition-colors">
        ...
        {error && (<p className="mt-3 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2">
            {error}
          </p>)}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={DollarSignIcon} label="Tổng doanh thu" value={formatCurrency(activeReport.revenue)}/>
        <StatCard icon={ShoppingCartIcon} label="Tổng đơn hàng" value={new Intl.NumberFormat('vi-VN').format(activeReport.orders)}/>
        <StatCard icon={TrendingUpIcon} label="Tăng trưởng" value={`${activeReport.growth >= 0 ? '+' : ''}${activeReport.growth.toFixed(1)}%`}/>
        <StatCard icon={PercentIcon} label="Tỷ lệ lợi nhuận" value={`${activeReport.profitMargin.toFixed(1)}%`}/>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={activeReport.categoryData} cx="50%" cy="45%" innerRadius={55} outerRadius={90} paddingAngle={3} dataKey="value">
                    {activeReport.categoryData.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                  </Pie>
      ...
      <div className="bg-white rounded-xl p-4 md:p-5 border border-gray-100 shadow-sm space-y-4">
        ...
        <DataTable title={`Hiệu suất sản phẩm trong báo cáo: ${activeReport.reportName}`} columns={productColumns} data={productTableData} searchPlaceholder="Tìm theo sản phẩm, danh mục..."/>
      </div>
      ...
      <div className="bg-white rounded-xl p-4 md:p-5 border border-gray-100 shadow-sm space-y-4">
        ...
        <DataTable title={`Chi tiết lợi nhuận theo sản phẩm: ${activeReport.reportName}`} columns={profitColumns} data={profitTableData} searchPlaceholder="Tìm theo sản phẩm, lợi nhuận..."/>
      </div>

      <DataTable title="Danh sách báo cáo đã tạo" columns={reportsColumns} data={reportTableData} searchPlaceholder="Tìm theo tên báo cáo, khoảng thời gian..." rowActions={[
            {
                key: 'view',
                label: 'Xem',
                onClick: (row) => setSelectedReportId(row.id),
                className: 'px-3 py-1.5 text-xs font-medium text-gold-600 hover:text-gold-700 hover:bg-gold-50 rounded-lg transition-colors',
            },
        ]}/>

      <DataTable title={`Chi tiết doanh số của báo cáo: ${activeReport.reportName}`} columns={monthlyColumns} data={monthlyTableData} searchPlaceholder="Tìm theo tháng, doanh thu, tăng trưởng..."/>
    </div>);
}
