...
const columns = [
  ...
  },
  {
    ...
    render: (val: string) => (
      <span
        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${val === 'Admin' ? 'bg-purple-100 text-purple-700' : val === 'Quản lý' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}
      >
        {val}
      </span>
    ),
  },
  {
    ...
    render: (val: string) => (
      <span
        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${val === 'Hoạt động' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
      >
        {val}
      </span>
    ),
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={!!editing}
              className={`w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400/50 ${editing ? 'bg-gray-50' : ''}`}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
