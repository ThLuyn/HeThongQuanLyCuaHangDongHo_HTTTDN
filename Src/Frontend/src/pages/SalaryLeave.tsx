...
const columns = [
    ...
    },
    {
        ...
        render: (val) => (<span className={`font-medium ${val <= 5 ? 'text-red-600' : 'text-green-600'}`}>
        {val} ngày
      </span>),
    },
];
export function SalaryLeave() {
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex items-center gap-4 bg-white rounded-xl px-6 py-4 shadow-sm border border-gray-100">
        ...
        <select value={month} onChange={(e) => setMonth(e.target.value)} className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400/50">
          {Array.from({
            length: 12,
        }, (_, i) => (<option key={i + 1} value={String(i + 1)}>
                Tháng {i + 1}
              </option>))}
        </select>
        <span className="text-sm text-gray-500">/ 2026</span>
      </div>
      <DataTable title={`Tính lương & Nghỉ phép - Tháng ${month}/2026`} columns={columns} data={data} searchPlaceholder="Tìm nhân viên..." onEdit={(row) => alert(`Chỉnh sửa lương cho ${row.name}`)}/>
    </div>);
}
