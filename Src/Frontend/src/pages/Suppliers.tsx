...
import { createSupplierApi, deleteSupplierApi, getSuppliersApi, updateSupplierApi, } from "../utils/backendApi";
const EMPTY_FORM = {
    name: "",
    address: "",
    phone: "",
...
export const Suppliers = () => {
    ...
    const addressOptions = useMemo(() => {
        ...
        suppliers.forEach((item) => {
            if (item.DIACHI)
                unique.add(item.DIACHI);
        });
        return Array.from(unique).sort((a, b) => a.localeCompare(b));
    }, [suppliers]);
    const filteredSuppliers = useMemo(() => {
        return suppliers.filter((item) => {
            if (addressFilter && item.DIACHI !== addressFilter)
                return false;
            if (!normalizedQuery)
                return true;
            const haystack = [
    ...
    return (<div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-xl border border-black/5 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm theo tên, mã, SĐT, email..." className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200 sm:w-72"/>
          <select value={addressFilter} onChange={(e) => setAddressFilter(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
            <option value="">Tất cả địa chỉ</option>
            {addressOptions.map((address) => (<option key={address} value={address}>
                {address}
              </option>))}
          </select>
          <button onClick={openCreate} className="rounded-lg bg-gold-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gold-600">
            + Thêm NCC
          </button>
        </div>
      </div>
      ...
      {loading ? (<div className="rounded-xl border border-black/5 bg-white p-6 text-center text-gray-600 shadow-sm">
          Đang tải danh sách nhà cung cấp...
            ...
            ]} data={filteredSuppliers} rowActions={[
      ...
      <Modal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} title="Chi tiết nhà cung cấp">
        {detailItem ? (<div className="space-y-2 text-sm text-gray-700">
            <p>
